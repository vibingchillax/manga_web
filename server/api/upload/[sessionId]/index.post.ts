...
export default defineEventHandler(async (event) => {
  ...
  if (data.length > MAX_FILES_PER_REQUEST)
    throw createError({
      statusCode: 400,
  ...
  const currentFileCount = sessionStats._count || 0;
  const currentTotalSize = sessionStats._sum?.fileSize || 0;
  ...
  for (const file of data) {
    try {
      if (currentFileCount + 1 > MAX_FILES_PER_SESSION) {
        throw new Error(
          `Session file limit exceeded (${MAX_FILES_PER_SESSION})`,
        );
      }
      ...
      if (currentTotalSize + file.data.length > MAX_TOTAL_SIZE_PER_SESSION) {
        throw new Error(
          `Session total size limit exceeded (${MAX_TOTAL_SIZE_PER_SESSION / (1024 * 1024)}MB)`,
        );
      }
      ...
      if (!mime || !IMAGE_TYPES.includes(mime)) {
        throw new Error(
          `Invalid or unrecognized file type: ${mime ?? "unknown"}. Allowed: JPEG, PNG, GIF.`,
        );
      }
      ...
      if (metadata.width! > MAX_WIDTH || metadata.height! > MAX_HEIGHT) {
        throw new Error(
          `Image dimensions exceed ${MAX_WIDTH}x${MAX_HEIGHT}. Actual: ${metadata.width}x${metadata.height}`,
        );
      }
