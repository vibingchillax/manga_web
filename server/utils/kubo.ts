...
export async function downloadImageFromUrl(url: string): Promise<DownloadResult> {
  try {
    ...
    if (!type.mime.startsWith('image/')) throw new Error(`Invalid MIME type: ${type.mime}`)
...
export async function downloadImagesFromUrls(urls: string[], concurrency = 15): Promise<DownloadResult[]> {
