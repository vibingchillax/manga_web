...
export function matchDomains(...domains: string[]): RegExp {
  ...
  return new RegExp(`https?:\\/\\/(?:www\\.)?(?:${escaped.join('|')})[\\/\\w-]*`, 'i')
}
...
export function createExternalSite(
  ...
  if (Array.isArray(domains) && domains.length > 0) {
    ...
    resolvedIcon =
      getFavicon(domains[0]!) || getFavicon('www.' + domains[0]) || 'i-lucide-circle-question-mark'
  } else if (typeof domains === 'string' && domains) {
    matcher = matchDomains(domains)
    resolvedIcon =
      getFavicon(domains) || getFavicon('www.' + domains) || 'i-lucide-circle-question-mark'
  }
...
export const EXTERNAL_SITES: ExternalSite[] = [
  {
    ...
    getData: url => /https?:\/\/kitsu\.app\/manga\/([\w-]+)/.exec(url)?.[1] ?? url,
  ...
  },
  {
    ...
    getData: url => /https?:\/\/anilist\.co\/manga\/(\d+)/.exec(url)?.[1] ?? url,
  ...
  },
  {
    ...
    getData: url => /https?:\/\/www\.anime-planet\.com\/manga\/([\w-]+)/.exec(url)?.[1] ?? url,
  ...
  },
  {
    ...
    getData: url => /^https?:\/\/www\.mangaupdates\.com\/series(.html\?id=|\/)(\w+)/.exec(url)?.[2] ?? url,
    constructUrl: id =>
      /^\d+$/.test(id)
  ...
  },
  {
    ...
    getData: url => /https?:\/\/www\.novelupdates\.com\/series\/([\w-]+)/.exec(url)?.[1] ?? url,
  ...
  },
  {
    ...
    getData: url => /https?:\/\/myanimelist\.net\/manga\/(\d+)/.exec(url)?.[1] ?? url,
  ...
  },
  {
    ...
    getData: url => /https?:\/\/(?:global\.)?bookwalker\.jp\/((?:series\/)?.+)/.exec(url)?.[1]?.replace(/\/$/, '') ?? url,
    constructUrl: id => `https://bookwalker.jp/${id}`,
  },

  createExternalSite('Amazon', 'amz', ['amazon.com', 'amazon.co.jp', 'amazon.cn'], 'brandAmazon'),
  createExternalSite('eBookJapan', 'ebj', 'ebookjapan.yahoo.co.jp', 'brandEBookJapan'),
  ...
  createExternalSite('Nico Nico', 'nicoVideo', 'nicovideo.jp', 'faviconSeigaNicovideoJp'),
...
export const BOOK_STORES = EXTERNAL_SITES.filter(s => ['amz', 'bw', 'ebj', 'cdj'].includes(s.apiCode))
export const STREAMING_SITES = EXTERNAL_SITES.filter(s => ['bt', 'cr', 'youtube'].includes(s.apiCode))
export const TRACKING_SITES = EXTERNAL_SITES.filter(s => ['al', 'ap', 'mal', 'mu', 'nu', 'kt'].includes(s.apiCode))
export const OFFICIAL_SOURCES = EXTERNAL_SITES.filter(s => ['raw', 'engtl'].includes(s.apiCode))
export const SOCIAL_LINKS = EXTERNAL_SITES.filter(s =>
  ['namicomi', 'twitter', 'pixiv', 'melonBook', 'fanBox', 'booth', 'nicoVideo', 'skeb', 'fantia', 'tumblr', 'youtube', 'naver', 'weibo', 'website'].includes(s.apiCode)
)