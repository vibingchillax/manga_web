...
export const ALPHABETICAL_LANGUAGES: { label: string, value: string }[] = LANGUAGES.filter(lang => lang.code !== "NULL")
  .map(lang => ({ label: lang.name.inEnglish, value: lang.code }))
  .sort((a, b) => a.label.localeCompare(b.label))


export const PREFERRED_ORDER = ["ja", "ja-ro", "ko", "ko-ro", "zh", "zh-hk", "zh-ro", "en"]
...
export const LANGUAGE_LISTS = {
  ...
  ORIGINAL_LANGUAGES: [
    ...PREFERRED_ORDER.map(code => toDropdownOption(LANGUAGES_BY_CODE.get(code))),
    ...ALPHABETICAL_LANGUAGES.filter(opt => !PREFERRED_ORDER.includes(opt.value)),
  ],
}

export function filterNonLatin(options: { label: string; value: string }[]) {
  return options.filter(opt => LANGUAGES_BY_CODE.get(opt.value)?.display.script !== "latin")
}
