...
const bannerStyle = computed(() => ({
  ...
  //   : '100%'
  width:
    $breakpoints.lg.value && menuActive.value ? "calc(100% - 256px)" : "100%",
}));

const hasGradient = computed(() =>
  ["/title", "/group", "/user", "/"].some((path) =>
...
<template>
  ...
  >
    ...
    <div class="banner-container" :class="{ block: isManga }">
      ...
      <div v-else-if="background" class="banner-image" :style="bannerStyle" />
      <div v-else class="banner-placeholder" :style="bannerStyle" />
      <div class="banner-shade" />
    </div>

    <div v-if="!isManga" class="clear-banner" />
    ...
    <div v-if="isManga" class="title">
      <template v-if="title || altTitle">
        <BigText
          extra-text-class="line-clamp-2 leading-5 pt-1"
          class="mb-1"
        ...
        <div class="flex flex-row gap-2">
          <div
            v-if="subTitle && !supportLink"
            class="font-normal text-xs sm:text-base truncate"
          >
            {{ subTitle }}
          </div>
    ...
    <div
      ...
      :class="[{ 'sm:ml-2': isManga }, 'relative']"
    >
      <slot name="buttons" />
    </div>
...
.layout-container.manga {
  --banner-overlay-gradient: linear-gradient(
    to bottom,
    rgb(var(--mw-background) / 0.8) 0%,
    rgb(var(--mw-background)) 100%
  );
  gap: 0.75rem 1rem;
...
@media not (min-width: 40rem) {
  .layout-container.has-gradient:not(.manga) {
    --banner-overlay-gradient: linear-gradient(
      to bottom,
      rgb(var(--mw-background)) 0%,
      rgb(var(--mw-background) / 0.8) 56px,
      rgb(var(--mw-background) / 0) 50%
    );
  }
}
