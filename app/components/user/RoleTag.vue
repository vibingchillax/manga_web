...
const roleMap: [UserRole, RoleStyle][] = [
  ['admin', { text: 'Admin', color: 'rgb(255,255,255)', background: '#9b59b6' }],
  ['groupLeader', { text: 'Group Leader', color: 'rgb(255,255,255)', background: '#3498db' }],
  ['user', { text: 'User', color: 'rgb(0,0,0)', background: 'rgb(250, 250, 250)' }],
  ['banned', { text: 'Banned', color: 'rgb(255,255,255)', background: 'rgb(0, 0, 0)' }],
]
...
<template>
  <div class="role-tag" :class="{ independent: bgIndependent }" :style="{
    backgroundColor: bgIndependent ? undefined : roleStyle.background,
    color: bgIndependent ? undefined : roleStyle.color,
  }">
    <div v-if="bgIndependent" class="role-dot" :style="{ backgroundColor: roleStyle.background }"></div>
    {{ roleStyle.text }}
  </div>
</template>
