...
const normalSizes = [
  "4.5rem", "4rem", "3.5rem", "3rem", "2.5rem",
  "2rem", "1.75rem", ...smallSizes
]
...
const resizeText = async () => {
  ...
  if (parentRect.width * availableHeight <= state.lastSize) {
    ...
    if (el.scrollHeight <= availableHeight && el.scrollWidth <= parentRect.width)
      return
  }
  ...
  const cloneTarget = cloneParent.querySelector(`.${markerClass}`) as HTMLElement
