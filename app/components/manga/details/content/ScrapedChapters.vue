...
import type { Manga, ScrapedChapter, ScrapedManga } from "~~/shared/types";
...
async function scrapeChapters() {
  ...
  try {
    const chapters = await $fetch(
      `/api/scraped/manga/${selectedManga.value.id}/feed`,
    );
...
const groupedChapters = computed(() => {
  ...
  const filteredChapters = scrapedChapters.value.filter(
    (ch) =>
      preferences.filteredLanguages.length === 0 ||
      preferences.filteredLanguages.includes(
  ...
  <div class="flex gap-6 items-start">
    <div class="mt-6">
      <USelectMenu
        ...
        placeholder="Select reading source"
      ...
      <UModal title="Reselect manga to scrape">
        <UButton
          ...
          variant="ghost"
          color="neutral"
          label="Incorrect match?"
        />
        <template #body>
          <div class="grid gap-2">
            <div
              v-for="manga in scrapedMangaList"
              :key="manga.id"
              class="cursor-pointer transition-all duration-200 rounded-xl border-2 overflow-hidden"
              :class="[
            ...
            >
              <MangaCard :manga="toManga(manga)" dense no-link show-flag />
            </div>
          </div>
        </template>
      </UModal>
    ...
    <div class="flex-grow mt-6">
      <UProgress
        ...
        :max="['Fetching manga...', 'Fetching chapters...']"
      />
      <div v-if="!loading && hasFetched && scrapedChapters.length > 0">
        <ChaptersList
          v-for="[vol, chapters] in groupedChapters"
          :key="vol"
          :volume="vol"
          :chapters="chapters"
        />
