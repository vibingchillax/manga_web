...
const filteredByLocale = computed(() => {
  return selectedLocales.value.length > 0
    ? allCovers.value.filter(
      c => selectedLocales.value.includes(c.attributes?.locale ?? '??')
    )
    : allCovers.value
})
...
const covers = computed(() => {
  ...
  const sorted = filteredByLocale.value
    .sort(
      (a, b) =>
        parseFloat(a.attributes?.volume ?? '0') - parseFloat(b.attributes?.volume ?? '0')
    )
    .map(c => ({
  ...
  if (sorted.length > 1 && sorted.some(c => !c.volume)) {
    sorted.splice(sorted.findIndex(c => !c.volume), 1)
  }
  ...
  <div v-else>
    ...
    <div v-else>
      <div class="mb-4">
        ...
        <USelect v-model="selectedLocales" :items="availableLocales"
          placeholder="Any locale" multiple :ui="{ content: 'min-w-fit' }" class="w-48">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-2">
        <MangaCover v-for="cover in covers" :key="cover.file" :manga="manga"
          :coverFile="`https://uploads.mangadex.org/covers/${manga.id}/` + cover.file"
          :label="cover.volume ? `Volume ${cover.volume}` : 'No volume'"
          :noTitle="false"
          lightbox :use256="false" />
      </div>
    </div>
  </div>
</template>