...
const { data: followStatus, pending } = await useFetch(
  `/api/manga/${props.manga.id}/follow`,
  {
    key,
  },
);
...
<template>
  ...
  <UModal
    ...
    :ui="{ content: 'preview-modal' }"
  >
    <template #body>
      <div class="text-sm pb-5 first:pt-4 px-4">
        <div class="preview-grid">
          <NuxtImg
            class="rounded shadow-lg width-limit self-start"
            style="grid-area: cover"
            :src="cover.url256"
            :alt="title"
          />
          <div style="grid-area: options">
            <div
              class="font-bold text-xl self-start mb-2"
              style="grid-area: title; word-break: break-word"
            >
              {{ title }}
            </div>
