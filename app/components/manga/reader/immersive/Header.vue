...
<template>
  <div v-if="!$breakpoints.sm.value && !immersive" class="non-app-mobile" :class="{
    break: immersionBreak,
    '!opacity-0': scrolling,
  }">
    <UButton variant="ghost" color="neutral" class="-ml-2 mr-2"
      :icon="immersive ? 'i-lucide-minimize' : 'i-lucide-expand'" size="sm"
      @click.prevent.stop="toggleImmersive" />
    <div>
      <NuxtLink v-if="chapterMeta.mangaLink || chapterState === 'pgonly'"
        :to="chapterMeta.mangaLink" class="reader--header-manga text-left">
        {{ chapterMeta.mangaTitle }}
      </NuxtLink>

      <USkeleton v-else class="h-5 mb-1 rounded" />

      <div v-if="chapterMeta.chapterTitle" class="reader--header-title text-left">
        {{ chapterMeta.chapterTitle }}
      </div>

      <USkeleton v-else-if="chapterState !== 'pgonly'" class="h-6 mb-1 rounded" />
    </div>

    <UButton class="ml-auto -mr-2" icon="i-lucide-menu" size="sm" variant="ghost" color="neutral"
