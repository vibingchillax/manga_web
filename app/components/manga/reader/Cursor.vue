...
const { start: startHideTimer, stop: stopHideTimer } = useTimeoutFn(() => {
  active.value = false;
  parent.value?.parentElement?.style &&
    (parent.value.parentElement.style.cursor = "none");
}, HIDE_DELAY);
...
const updateCursor = (e: MouseEvent) => {
  ...
  const arrowRight =
    readStyle.value === ReadStyleEnum.RTL
...
.cursor.dark {
  ...
  filter: drop-shadow(3px 5px 5px rgb(255 255 255/0.8));
}
