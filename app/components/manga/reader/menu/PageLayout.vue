...
const { greyscale } = storeToRefs(reader);
...
const progressModeStr = computed({
  ...
  set: (val: string) =>
    settings.setProgressMode(Number(val) as ProgressModeEnum),
});

const progressSideStr = computed({
  get: () => String(progressSide.value),
  set: (val: string) =>
    settings.setProgressSide(Number(val) as ProgressSideEnum),
});
...
const backgroundColorModel = computed({
  get: () =>
    backgroundColor.value === "transparent" ? undefined : backgroundColor.value,
  set: (val: string) => {
...
<template>
  ...
  Progress Size
  <USlider v-model="progressHeight" :min="2" :max="16" tooltip />
  Cursor Action Hints
  ...
  <div>
    <UColorPicker v-model="backgroundColorModel" format="rgb" />
    <UButton @click="settings.setBackgroundColor('transparent')"
      >Transparent</UButton
    >
