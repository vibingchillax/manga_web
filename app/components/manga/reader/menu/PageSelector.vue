...
<template>
  <div class="flex">
    <template v-if="pageState === 'loaded'">
      <UButton icon="i-lucide-chevron-left" :disabled="pageOptions.length === 0"
        @click="() => incrementPageGroup(-1, router)" />
      <USelect v-model="currentPageGroup" :items="pageOptions" @update:model-value="val => setCurrentPageGroup(val)" />
      <UButton icon="i-lucide-chevron-right" :disabled="pageOptions.length === 0"
        @click="() => incrementPageGroup(1, router)" />
    </template>
    <template v-else>
