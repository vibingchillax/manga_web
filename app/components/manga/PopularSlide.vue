...
<template>
    <a class="flex relative h-full overflow-hidden shadow banner-bg" :href="`/title/${manga.id}/${toKebabCase(title!)}`"
        style="height: 400px;">
        <!-- dont do var! lol this is temp-->
        <img class="absolute left-0 top-0 w-[100%] h-[150%] object-cover select-none" :src="coverUrl" alt=""
        ...
        <div
            ...
            <div class="h-full relative z-20 flex gap-4">
                <a :href="`/title/${manga.id}/${toKebabCase(title!)}`"
                    ...
                    <img class="rounded shadow-md w-full h-full" :src="coverUrl" alt="Cover image">
