...
const idList = computed(
  () =>
    data.value?.data?.relationships
      ?.filter(
        (r): r is { id: string; type: "manga" } => r.type === "manga" && !!r.id,
      )
      .map((r) => r.id) ?? [],
);
...
<template>
  <div class="header overflow-hidden">
    <div class="flex justify-between items-center text-2xl mb-4">
      ...
      <NuxtLink
        class="custom-opacity relative flex items-center overflow-hidden accent text rounded-full !px-0"
        to="/titles/seasonal"
      >
    ...
    <ClientOnly>
      ...
      >
        <swiper-slide
          v-for="(manga, index) in mangaList?.data"
          :key="index"
          style="max-width: 192px; flex: 0 0 auto"
        >
          <div>
            <MangaCover
              :manga="manga"
