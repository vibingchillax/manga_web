...
<template>
  <NuxtLink
    class="flex relative h-full overflow-hidden shadow banner-bg"
    :href="detailsUrl"
    style="height: 400px"
  >
    <NuxtImg
      class="absolute left-0 top-0 w-[100%] h-[150%] object-cover select-none"
      :src="cover.urlOriginal"
      alt=""
      loading="lazy"
      style="object-position: 0px 30%"
      placeholder
    />
    <div class="absolute banner-bg inset-0" />
    <div
    ...
    >
      <div class="h-full relative z-20 flex gap-4">
        ...
        />
        <div
          class="mt-auto grid gap-6 sm:gap-2 h-full"
          style="
            min-height: 0px;
        ...
        >
          <h2
            class="font-bold text-xl line-clamp-5 sm:line-clamp-2 lg:text-4xl overflow-hidden"
            style="line-height: 2.75rem"
          >
            {{ title }}
          </h2>
          <TagsRow :rows="1" class="overflow-hidden">
            <!-- bp ? 1 : 2-->
            <MangaTag v-if="contentRating" :value="contentRating" />
            <MangaTag
              v-for="tag in sortedTags(tags)"
              :key="tag.id"
