...
<template>
  <component :is="input ? 'label' : 'div'" :for="input" class="page" :class="{
    ...
    <template v-if="dragging"></template>
    ...
    <template v-else>
      ...
      <div v-if="state !== UploadState.Fail && message && !input" class="info-icon">
        <UTooltip :disabled="!message" :text="message">
          <UButton size="xs" variant="soft" class="rounded-full" color="neutral"
            icon="i-lucide-info" />
        </UTooltip>
      </div>

      <div class="flex flex-col gap-2 absolute -top-2 -left-2">
        <label v-if="!disabled && !input" for="edit-file" class="edit-page relative"
        ...
        <UTooltip v-if="state === UploadState.Fail && !input" :disabled="!failReason"
          :text="failReason">
          <div class="rounded-full text-white relative w-6 h-6 bg-danger flex
            justify-center items-center">
            <UButton size="xs" variant="soft" color="neutral" icon="i-lucide-file-warning"
              class="rounded-xl" />
          </div>
        </UTooltip>
      </div>
      ...
      <div v-if="!input && !noDrag" class="middle">
        <UButton v-if="state === UploadState.Fail" color="error" icon="i-lucide-x" />
        <Loading v-else-if="state === UploadState.Pending || state === UploadState.Uploading"
          color="primary" :compact="compact" />
        <UIcon v-else-if="disabled && state === UploadState.Success" color="green"
          name="i-lucide-check" />
...
.page.dragging {
  animation: wiggle-8c173c17 .3s ease-in-out infinite
}
