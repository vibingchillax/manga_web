...
<template>
  ...
  <div class="flex justify-left flex-wrap gap-2">
    <UCheckbox
      v-show="pages.length >= 75"
      v-model="state.showPages"
      class="w-full mb-2"
    />
      ...
      <template #item="{ element }: { element: UploadPage }">
        <div class="flex-grow-0 p-3" :class="{ 'draggable-page': !disabled }">
          <MangaUploadImage
            ...
            :is-duplicate="element.isDuplicate"
            :preview="element.preview"
            :state="element.state"
            :fail-reason="element.failReason"
            :message="element.message"
            :file-name="element.file?.name"
            :progress="element.progress"
            :compact="!state.showPages && pages.length >= 75"
            @remove="$emit('removeFile', element)"
            @edit-click="($emit('editClick', element), editFileInput?.click())"
          />
    ...
    <Overlay v-if="dragTimeout" attach="body">
      <UTooltip> Drop files anywhere to upload! </UTooltip>
    </Overlay>
  </div>
</template>
