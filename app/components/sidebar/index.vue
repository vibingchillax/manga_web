...
const sections = computed<Section[]>(() => {
  const home: Section = [{ title: "Home", icon: "i-lucide-house", link: "/" }];
...
<template>
  <div class="flex flex-col bg-accent h-screen md:h-full z-[7] lg:z-auto">
    ...
    >
      <div
        class="lg:top-0 lg:sticky flex flex-col overflow-y-auto overscroll-contain h-screen"
      >
        <div class="mx-4 py-2 flex-shrink-0 flex justify-between items-center">
          <NuxtLink to="/">
            <USkeleton class="h-12 w-48 flex justify-center items-center"
              >manga_web</USkeleton
            >
          </NuxtLink>
          <UButton
        ...
        <div
          ...
          :id="`section-${section[0]?.title.replace(/\W+/g, '_')}`"
          :key="section[0]?.title"
          class="px-4 pt-2 flex flex-col flex-shrink-0"
        >
          <template v-for="entry in section" :key="entry.title">
            <SidebarItemLink v-if="'link' in entry" v-bind="entry" />
            <SidebarItemHeader v-else v-bind="entry" />
          </template>
        </div>
        <div class="flex-grow" />
