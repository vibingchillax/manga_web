...
const activeTab = ref<
  "reading" | "planToRead" | "completed" | "onHold" | "rereading" | "dropped"
>("reading");
...
<template>
  <Page title="Library" wide require-auth>
    ...
    <div class="flex my-4 flex-row justify-between gap-6">
      <div class="text-lg">{{ filteredManga.length }} title(s)</div>
    ...
    <div>
      ...
      >
        <MangaCard
          v-for="(manga, index) in filteredManga"
          :key="manga.id"
          :manga="manga"
          :dense="active === 'dense'"
          :cover-only="active === 'coverOnly'"
          :use256="active !== 'coverOnly'"
          show-flag
        />
      </div>
    </div>
