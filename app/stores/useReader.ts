...
export const useReaderStore = defineStore("reader", {
  ...
  actions: {
    async initializeReader(
      ...
      if (!this.manga) {
        const manga = this.currentChapter.relationships?.find(
          (r) => r.type === "scraped_manga",
        );
        if (manga && "attributes" in manga) {
          this.manga = manga;
    ...
    async switchChapter(
      ...
      keepCurrentPage: boolean = false,
    ) {
      const readerCache = useReaderCache();
      if (!this.manga || !chapterId || chapterId == this.currentChapter?.id)
        return false;
    ...
    async _setAdjacentChapters() {
      ...
      this._nextChapter =
        getAdjacentChapter(
          ...
          this.currentChapter?.attributes.chapter,
          1,
        ) ?? null;
      this._prevChapter =
        getAdjacentChapter(
    ...
    goToPage(pageNumber: number, chapterId?: string) {
      ...
      while (
        groupIndex < pageManager.pageGroups.length &&
    ...
    async _markChapterRead(chapter: ScrapedChapter) {
      ...
      readMarker.setMarkers(this.manga?.id!, [
        { chapterId: chapter.id, type: "scraped" },
      ]);
    },
    setCurrentPageGroup(groupIndex: number, router?: Router) {
      ...
      if (
        !readerMenu.doAutoAdvance ||
        !this.adjacentPopulated ||
        direction === 0
      ) {
        if (direction === 0) {
        ...
        pageManager.pageGroups[this.currentPageGroup]?.forEach((p) =>
          p.retry(),
        );

        if (
          groupIndex >
          pageManager.pageGroups.length - 1 - PRELOAD_THRESHOLD
        ) {
      ...
      if (router && this.adjacentPopulated) {
        ...
        if (direction === -1) {
          ...
          this.switchChapter(this.chapterMeta.prevChapter?.id, true, true).then(
            onSwitchComplete,
          );
        } else if (direction === 1) {
          debug("Jumping to next chapter");
          this.currentPageGroup = pageManager.pageGroups.length - 1;
          this.switchChapter(this.chapterMeta.nextChapter?.id, true, true).then(
            onSwitchComplete,
          );
        }
      }
    ...
    incrementPageGroup(step: number, router?: Router, useLTR: boolean = false) {
      const direction =
        useLTR || useReaderMenu().readStyle === ReadStyleEnum.LTR ? 1 : -1;
      this.setCurrentPageGroup(
        this.currentPageGroup + step * direction,
        router,
      );
    },
    ...
    showImagePage(pageGroup: number) {
      const readerMenu = useReaderMenu();
      return (
        readerMenu.viewStyle === ViewStyleEnum.LongStrip ||
  ...
  getters: {
    ...
    chapterState() {
      ...
      if (
        this.chapterLoadError?.status === 404 &&
        this.serverLoadError.status === 404
      ) {
        return "error404";
      }
      if (
        this.chapterLoadError?.status === 404 &&
        useReaderPageManager().imageData
      ) {
        return "pgonly";
      }
